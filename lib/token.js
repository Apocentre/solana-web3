...
export const fetchMintTokensByOwner = async (
  self,
  account,
  page=1,
  size=10
...
export const wrapSolIXs = async (self, wallet, lamports) => {
  const ixs = []

  // 1. Create ATA account Ix is needed. This ATA will hold the wrapped SOL
  const [wrappedAtaIx, wrappedAta] = await self.createATAIxIfNeeded(wallet, spl.NATIVE_MINT)
  wrappedAtaIx && ixs.push(wrappedAtaIx)

  // 2. Create a Transfer SOL to the above account IX
  ...
  // 3. Native Sync ix
  ixs.push(
    spl.createSyncNativeInstruction(associatedTokenAccount)
  )

...
export const getComputationBudgerIx = (_, units) => {
...
export const createATAIx = async (self, user, mint) => {
  const ata = await self.getAssociatedTokenAddress(mint, user, true)
  const ix = await self.createATAIx(
    user,
    ata,
    user,
    mint
...
export const createATAIxIfNeeded = async (self, user, mint) => {
  let ix
